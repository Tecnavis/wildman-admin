...
function Dishes() {
  ...
  const handleEdit = async (id) => {
    ...
    try {
      ...
      setStocks(Object.fromEntries(Object.entries(data.images || {}).map(([color, images]) => [color, images[0]?.stock || 0])));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const updateDishes = async () => {
    ...
    try {
      const response = await axios.put(`${backendUrl}/admin/product/${uid}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${token}`,
        },
                  ...
                  {currentData.length === 0 ? (
                    ...
                    currentData.map((items, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-black item-text">
                          <Tooltip title="Edit">
                            <FiEdit onClick={() => handleEdit(items._id)} className="edit-icon" />
                          </Tooltip>
        ...
        <div className="container">
          <form>
            ...
            <input 
              ...
              onChange={isEditing ? handleUpdateChange : (e) => setItemnumber(e.target.value)} 
            />
            ...
            <input 
              ...
              onChange={isEditing ? handleUpdateChange : (e) => setDishes(e.target.value)} 
            />
            ...
            <input 
              ...
              onChange={isEditing ? handleUpdateChange : (e) => setOldPrice(e.target.value)} 
            />
              <label>New Price</label>
              <input type="number" className="form-control" placeholder="Enter New Price"
               value={isEditing ? getDishesById.newprice : newprice} 
               onChange={isEditing ? handleUpdateChange : (e) => setNewPrice(e.target.value)} />
              
              <label>Description</label>
              <textarea className="form-control" 
              value={isEditing ? getDishesById.description : description} 
              onChange={isEditing ? handleUpdateChange : (e) => setDescription(e.target.value)}
              />
              
              <label>Manufacturer</label>
              <textarea type="text" className="form-control" placeholder="Enter Manufacturer" 
              value={isEditing ? getDishesById.manufacturer : manufacturer} onChange={isEditing ? handleUpdateChange : (e) => setManufacturer(e.target.value)} />
              
              <label>Product Care</label>
              <textarea type="text" className="form-control" placeholder="Enter Product Care"
              value={isEditing ? getDishesById.productcare : productcare} onChange={isEditing ? handleUpdateChange : (e) => setProductcare(e.target.value)}
              />
              
              <label>Features</label>
              <textarea type="text" className="form-control"
              value={isEditing ? getDishesById.features : features} onChange={isEditing ? handleUpdateChange : (e) => setFeatures(e.target.value)}/>
